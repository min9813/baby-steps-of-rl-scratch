...
class DeepQNetworkTorch(nn.Module):

    def __init__(self, input_dim, output_dim) -> None:
        super().__init__()
        self.conv1 = nn.Conv2d(input_dim, 32, kernel_size=8, stride=4, padding=1)
...
class DeepQNetworkAgent(FNAgent):
    ...
    def make_model(self, feature_shape):
        ...
        #                          kernel_initializer=normal))
        model = DeepQNetworkTorch(input_dim=feature_shape[0], output_dim=len(self.actions))
        self.model = model
    ...
    def update(self, experiences, gamma):
        ...
        estimateds = torch.from_numpy(estimateds).float().to(self.device)

        self.optimizer.zero_grad()
        predict_values = self.model(states)
        loss = self.criterion(predict_values, estimateds)
    ...
    def update_teacher(self):
        ...
        self._teacher_model = copy.deepcopy(self.model)
